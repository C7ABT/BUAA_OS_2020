                             ...
                             u_char *bin, u_int32_t bin_size, void *user_data)
{
    ...
    for (i = 0; i < bin_size; i += BY2PG) {
        ...
        if(!i){
            if(BY2PG - offset < bin_size){
                ...
                bcopy((void *)bin, (void *)page2kva(p) + offset, bin_size);
            }
            }
        else{
            if((BY2PG - offset) < (bin_size - i)){
                ...
                bcopy((void *)bin + i - offset, (void *)page2kva(p), bin_size - i + offset);
            }		
        }
    ...
    * hint: variable `i` has the value of `bin_size` now! */
    while (i < sgsize) {
        ...
        if(r = page_insert(env->env_pgdir, p, va + i, (PTE_V | PTE_R))){
            return -E_NO_MEM;
        }
        i += BY2PG;
    }
...
load_icode(struct Env *e, u_char *binary, u_int size)
{
	...
	page_insert(e->env_pgdir, p, USTACKTOP - BY2PG, (PTE_R | PTE_V));

    /*Step 3:load the binary using elf loader. */
	if(r = load_elf(binary, size, &entry_point, e, load_icode_mapper)){
		return;
	}
...
env_run(struct Env *e)
{
    ...
    *  switch the context and save the registers. You can imitate env_destroy() 's behaviors.*/
	struct Trapframe *prev = (struct Trapframe *)(TIMESTACK - sizeof(struct Trapframe));
	if(curenv){
		curenv->env_tf = *prev;
