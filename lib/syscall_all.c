...
int unvalid_pos(u_int pos) {
    return ((pos < 0) || pos >= UTOP)?1:0;
}    
...
int sys_mem_alloc(int sysno, u_int envid, u_int va, u_int perm)
{
    ...
    if (unvalid_pos(va)) {
        return -E_INVAL;
    } else if (((perm & PTE_V) == 0) || (perm & PTE_COW)) {
        return -E_INVAL;
    }   else {
        if (ret = envid2env(envid, &env, 0)) {
            return ret;
        }   else if (ret = page_alloc(&ppage)) {
            return ret;
        }   else if (ret = page_insert(env->env_pgdir, ppage, va, perm)) {
            return ret;
        }
    }
    return 0;
				...
				u_int perm)
{
    ...
    //your code here
    if (unvalid_pos(srcva) || unvalid_pos(dstva) || !(perm & PTE_V)) {
        return -E_INVAL;
    }
