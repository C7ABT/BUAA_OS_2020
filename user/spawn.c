...
usr_load_elf(int fd , Elf32_Phdr *ph, int child_envid){
    ...
    u_int size_segment = ph->p_memsz, size_binary = ph->p_filesz, offset = ph->p_offset;
    void *temp;
    int i = ROUNDDOWN(va, BY2PG) - va, r;
    for (; size_binary > BY2PG &&  size_binary > BY2PG + i; i += BY2PG) {
        if((r = read_map(fd, offset + i, &temp)) < 0) {
            return r;
        }
        va_temp = va + i;
        syscall_mem_map(0, temp, child_envid, va_temp, PTE_V | PTE_R);
    }
    if (i < size_binary) {
        r = read_map(fd, offset+i, &temp);
        u_int partial = size_binary - i;
        if (size_binary > i) {
            user_bzero(((u_char*)temp + size_binary - i), (BY2PG - size_binary + i));
        }
        if(r < 0) {
            return r;
        }
    ...
    for (; i < size_segment; i += BY2PG) {
        va_temp = va + i;
        syscall_mem_alloc(child_envid, va_temp, PTE_V | PTE_R);
    }
...
{
    u_char elfbuf[512];
    int r, fd, size, text_start;
    u_int child_envid, i, esp, *blk;
    Elf32_Ehdr* elf = NULL;
    Elf32_Phdr* ph = NULL;
    ...
    if ((child_envid = syscall_env_alloc()) < 0) {
        user_panic("Can't alloc for this env, Mfcker\n");
        return child_envid;
    }
    ...
    u_char* binary = fd2data(num2fd(fd));
