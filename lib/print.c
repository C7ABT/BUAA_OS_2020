...
#define 	OUTPUT(arg, s, l)  \
  { if (((l) < 0) || ((l) > LP_MAX_BUF)) { \
    ...
    int longFlag; // if it's long
    int negFlag;  // if it's a negative number
    int width;	  // the width of output
    int prec;	  // the precision of number
    int ladjust;  // whether to output by the LEFT side
    char padc;	  // the particular SYMBOL used to fulfill EMPTY spaces
